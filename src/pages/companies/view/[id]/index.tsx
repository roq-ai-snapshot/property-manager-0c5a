...
import { deletePropertyById } from 'apiSdk/properties';
...
function CompanyViewPage() {
  ...
  const propertyHandleDelete = async (id: string) => {
    setDeleteError(null);
    try {
              ...
              {hasAccess('property', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    ...
                    <NextLink passHref href={`/properties/create?company_id=${data?.id}`}>
                  ...
                  <TableContainer mb={4}>
                    <Table variant="simple">
                      ...
                      <Tbody>
                        {data?.property?.map((record) => (
                          <Tr
                            cursor="pointer"
                            onClick={() => router.push(`/properties/view/${record.id}`)}
                            key={record.id}
                          >
                            ...
                            <Td>
                              {hasAccess('property', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                                <NextLink passHref href={`/properties/edit/${record.id}`}>
                              ...
                              {hasAccess('property', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                                <IconButton
                                  onClick={() => propertyHandleDelete(record.id)}
                                  colorScheme="red"
                                  variant="outline"
                                  aria-label="edit"
                                  icon={<FiTrash />}
